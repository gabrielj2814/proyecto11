...
if(!(isset($_SESSION["nombre_usuario_software"]))){
    ...
    $submenu_actual="atencion_diaria";
    $seccion_comentarios = $comentarios['atencion_diaria'];//mis cuotas
    $demo_seccion = $demo['atencion_diaria'];
...
<html lang="es"> 
    <head>
        ...
        <title><?php echo $nombre_pestana_navegador;?> | Diaria</title>
...
<!--------------------------------  MODAL INFORME PDF INICIO-------------------------------------------->
                    ...
                    <div id="component_atencion_diaria_formulario" style="width: 100%; height: 100%; display:none;">   
                        <div class="cabezera_formulario" style="background-color: #5cbde2;/* border-top-right-radius: 5px; *//* border-top-left-radius: 5px; */height: 12%;">
                            <center><img src="../config/logo_equipo.png" style="height: 65px;position: relative;top: -4px;"></center>
                        </div>
                        ...
                        <div class="" style="background-color: #eeeeee;width: 100%; height: 100%; max-height: 100%; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px;">
                            ...
                            <div style="margin-bottom:30px;">
                                <p class="titulo_formulario"  style="width: 100%;text-align:center;font-size:1.3em;">REGISTRAR ATENCIÓN</p>
                            </div>
                            <br>
                                ...
                                <form id="filtro_modal_formulario" >
                                    ...
                                    <div style="background-color: lightblue; width: 100%; margin: auto;">
                                        <div class="span3" style="display: flex">
                                            ...
                                            <select style="width:50%; height: 30px;background:#fff;border:2px solid" class="" id="serie_jugador" name="serie_jugador" onchange="consultarJugadoreXSeries(this.value)">
                                            </select>
                                            <!-- <input type="text" readonly  style="width:50%; height: 18px;background:#fff;" class="grey-input date_fechaNacimiento fecha_inicio" id="fecha_atencion" name="fecha_atencion" onchange="" />                    -->
                                        </div>
                                    </div>
                                    <div style="background-color: lightblue; width: 100%; margin: auto;" id="contenedor_jugador">
                                        <div class="span3" style="display: flex">
                                            ...
                                            <select style="width:50%; height: 30px;background:#fff;border:2px solid" class="" id="jugador" name="jugador" onchange="mostrarJugador(this.value)"></select>
                                            <!-- <input type="text" readonly  style="width:50%; height: 18px;background:#fff;" class="grey-input date_fechaNacimiento fecha_inicio" id="fecha_atencion" name="fecha_atencion" onchange="" />                    -->
                                        </div>
                                    </div>
                                </form>
                                ...
                                <div style="display:flex;flex-direction:row;flex-wrap:wrap;width:100%;height: 270px;/*background:green*/background:#eee;margin-bottom:15px;">
                                    <div style="width:20%;background:#eee;"><!--contendor foto-->
                                        <div style="width:100%;height:100%;display:flex;flex-direction:column;flex-wrap:wrap;justify-content:felx-start;align-items:center;">
                                            ...
                                            <div style="font-size: 1.2em;margin-top: 10px;width:80%;background:#eee;text-align:center;font-weight:bold;color:#404040;border-top:5px solid #404040;border-bottom:5px solid #404040">
...
var html_atencion_nuevo_incidente={
    ...
    <div style="width:30%;display:flex;margin-right:0px;margin-bottom:10px">\
        ...
        <div class="btn-group c_objetivo_fisico " style="width: 49%;">\
            <button id="boton_examenes_solicitados" type="button" class="btn dropdown-toggle" data-toggle="dropdown" href="#" onclick="renderizarCheckboxExamenesSolicitados()" style="width: 100%;height: 30px;border-radius: 0;border: 2px solid #404040; background-color: #fff;">\
    ...
    \
        ...
        <div class="tarjeta" style="width:31.6%;margin-left:1.2%;height: 350px;">\
            ...
            <div class="cuerpo_tarjerta" style="overflow: auto;overflow-x: hidden;height: 325px;">\
                ...
                <div style="width:100%;height:auto;display:block;" id="contenedor_radios_sesion">\
                </div>\
            </div>\
        </div>\
        <div style="width:97%;height:250px;margin-top:35px;margin-left: 1.5%;box-sizing: border-box;">\
            <div style="width:100%;height:30px;background:#404040;text-align: left;box-sizing: border-box;padding-top: 7px;color: #fff;border-radius: 5px 5px 0px 0px;padding-left: 7px;">Observaciones Kinesiologo</div>\
            <textarea id="observaciones_kinesiologo" name="observaciones_kinesiologo" onKeyup="validarFormulario()" onKeydown="validarFormulario()" style="width:100%;height:80%;border-radius: 0px 0px 5px 5px;resize: none;box-sizing: border-box;"></textarea>\
        </div>\
        <div class="row-fluid">\
            <div class="span12"  style=" margin-top: 20px;">\
                <center>\
...
function checkboxNingunoExamen(){
    ...
    validarFormulario();
    // console.log(array_examen_solicitado)
}
...
function consultarTratamiento2(){
    ...
    $.ajax({
        url: 'post/atencion_diaria_consultar_tratamiento.php',
        type: "post",
...
function consultarContextosIncidente2(){
    ...
    $.ajax({
        url: 'post/atencion_diaria_consultar_contexto_incidente.php',
        type: "post",
        success: function(respuesta) {
            let json=JSON.parse(respuesta);
...
function consultarTrabajoReadaptador2(){
    ...
    $.ajax({
        url: 'post/atencion_diaria_consultar_trabajo_readaptador.php',
        type: "post",
        success: function(respuesta) {
            let json=JSON.parse(respuesta);
            // console.log(json.datos.length);
...
function tablaNuevoIncidente(atencion_diaria){
    ...
    // console.log(lista_contexto_incidente_modal)
    // console.log(lista_tratamiento_aplicado_modal)
    // console.log(lista_trabajo_readaptador_modal)
    // alert(atencion_diaria.fecha_atencion_diaria)
...
async function descargarPDF(posicion){
    ...
    $.ajax({
        url: "post/reportes/generarPDF_ODR_tencion_diaria_jugador.php",
        type: "post",
        cache: false,
...
async function descargarListaPDF(){
    ...
    for(let atencion of window.busqueda_respuesta_servidor){
        lista_id_atencion_diaria.push({name:"array_id_atencion_diaria[]",value:atencion.idatencion_diaria});
    }
    ...
    $('#mensaje_agregar_descargarPDF').html('<h5><i class="icon-spinner icon-spin icon-large"></i> Generando PDF...</h5><br><img src="../config/agregar_archivo.png">');
    $.ajax({
        url: "post/reportes/generarPDF_ODR_tencion_diaria_lista_atenciones_jugadores.php",
        type: "post",
        cache: false,
...
function eliminarAtencionDiaria(id){
    ...
    $.ajax({
            url: "post/atencion_diaria_eliminar.php",
            type: "post",
            data:datos,
            success: function(respuesta) {
...
function buscarJugador(){
    ...
    $.ajax({
            url: "post/atencion_diaria_consultar.php",
            type: "post",
            data:datos_filtro,
            success: function(respuesta) {
...
function crearFilasTabla(datos){
    ...
    let filas=datos.map(atencion_diaria=>{
        ...
        const fila='\
        <tr class="panel_buscar" style="cursor:pointer; color:#404040; font-size:12px;" id="atencion_diaria_'+atencion_diaria.idatencion_diaria+'">\
            ...
            <td style="text-align:left;" onClick="verAtencionDiaria('+numero_fila+');">\
            <div style="display:block;float: left;margin-right: 5px;border:2px solid #404040;border-radius: 100px;width:22px;height:22px;"><img style="border-radius: 100px;width:100%;height:100%" src="./foto_jugadores/'+atencion_diaria.idfichaJugador+'.png" style="width:100%;height:100%;"/></div>\
            ...
            <td style="text-align:left;" onClick="verAtencionDiaria('+numero_fila+');">\
                <div style="max-width:75px">\
                    <p class="ellipsis-text" style="font-weight: normal;font-size:10px;">\
                        '+formatoRut(atencion_diaria.rut)+'\
                    </p>  \
                </div>\
            </td>\
            <td style="text-align:left;" onClick="verAtencionDiaria('+numero_fila+');">\
            ...
            <td style="text-align:center;" onClick="verAtencionDiaria('+numero_fila+');">\
                <div style="max-width:90px">\
                    <p class="ellipsis-text" style="color:#fff;'+((atencion_diaria.recomendacion_sesion_actual_atencion_diaria ==="10")?"background-color:green;":"background-color:red;")+'font-size:10px;">\
                        '+((atencion_diaria.recomendacion_sesion_actual_atencion_diaria  ==="10")?"Si":"No")+'\
                    </p>  \
                </div>\
            </td>\
            <td style="text-align:left;" onClick="verAtencionDiaria('+numero_fila+');">\
            ...
            <td style="padding:2px;">\
                ...
                <td style="padding:2px;">\
                    <center>\
                        <a class="boton_eliminar" onClick="abrirModalEliminarAtencionDiaria('+atencion_diaria.idatencion_diaria+');">\
...
function obtenerTipoDeAtencion(tipo){
    var lista_tipo_atencion=[
        {numero_tipo_atencion:1,nombre_tipo_atencion:"Nuevo"},
        {numero_tipo_atencion:2,nombre_tipo_atencion:"Control"},
...
async function mostrarModalFormularioEditar(posicion){
    ...
    window.id_atencion_diaria=atencion_diaria.idatencion_diaria;
    $("#serie_jugador").empty();
    let series_array=[
    //serie masculina
    {numero_serie:"0",nombre_serie:"Seleccione una serie"},
...
async function consultarJugadoreXSeries(serie_select){
    ...
    if(serie_select!=="0"){
        ...
        await $.ajax({
                url: "post/atencion_diaria_consultar_jugador.php",
                type: "post",
                data:datos,
                success: function(respuesta) {
                    var json=JSON.parse(respuesta);
...
function agregarNuevoTratamiento(){
    ...
    $.ajax({
        url: "post/atencion_diaria_agregar_tratamiento.php",
        type: "post",
        data:datos_backend,
        success: function(respuesta) {
            let json=JSON.parse(respuesta) ; 
...
function agregarNuevoTrabajoReadaptador(){
    ...
    $.ajax({
        url: "post/atencion_diaria_agregar_trabajo_readaptor.php",
        type: "post",
        data:datos_backend,
        success: function(respuesta) {
            let json=JSON.parse(respuesta) ; 
...
//  zonas_bck
    ...
    let estado_observaciones_kinesiologo=validarCamposVacios2("observaciones_kinesiologo") ; 
    //
    //select
    ...
    if( estado_diagnostico && estado_anamnesis){
    ...
    //validacion final
    if((zonas_frt.length>0 || zonas_bck.length>0) && estado_campo_texto && estado_tratamiento_aplicado && estado_select){
        if(estado_examenes_solicitados){
...
function enviarDatos(){
    ...
    if(tipo_atencion_formulario==="1"){
        ...
        if(estado_checkbox_tratamiento){
            let contador1=0 ; 
            ...
            $.ajax({
                url: "post/atencion_diaria_guardar.php",
                type: "post",
