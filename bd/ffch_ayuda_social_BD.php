...
function ver_datos( $datos ){
    ...
    switch( $tipo_consulta ) {
        ...
        case 'get_jug_ultima_ayudasocial_sexoserie':
            ...
            $anio_filtro_busqueda = intval( $datos['anio_filtro_busqueda'] );
            $mes_filtro_busqueda = intval( $datos['mes_filtro_busqueda'] );
            $idclub_filtro_busqueda = intval( $datos['idclub_filtro_busqueda'] );
            $tipoayuda_filtro_busqueda = $datos['tipoayuda_filtro_busqueda'];  
            
            if( $anio_filtro_busqueda === 0 ) {
                $anio_filtro_busqueda = "1 = 1";
            } else {
                $anio_filtro_busqueda = "YEAR(informe_ayuda_social.fecha_ayuda_social) =  ".$anio_filtro_busqueda."";
            }

            if( $mes_filtro_busqueda === 0 ) {
                $mes_filtro_busqueda = "1 = 1";
            } else {
                $mes_filtro_busqueda = "MONTH(informe_ayuda_social.fecha_ayuda_social) = ".$mes_filtro_busqueda."";
            }            

            if( $idclub_filtro_busqueda === 0 ) {
                $idclub_filtro_busqueda = "1 = 1";
            } else {
                $idclub_filtro_busqueda = "fichaJugador_club.idclub = ".$idclub_filtro_busqueda."";
            }  
            ...
            while( $row = mysqli_fetch_assoc( $resultado ) ) {
                ...
                // Consultar último informe de ayuda social:
                $resultado_2 = $link->query("
                    SELECT * FROM informe_ayuda_social 
                    WHERE idfichaJugador = ".$idfichaJugador."
                    AND ".$anio_filtro_busqueda."   
                    AND ".$mes_filtro_busqueda."
                    ORDER BY informe_ayuda_social.idinforme_ayuda_social DESC
                    LIMIT 1 
                ");

                while($row2 = mysqli_fetch_assoc($resultado_2)) {
                    // Datos de la tabla 'informe_ayuda_social':
                    $row['idinforme_ayuda_social'] = $row2['idinforme_ayuda_social'];
                    $row['fecha_ayuda_social'] = $row2['fecha_ayuda_social'];
                    $row['monto_ayuda_social'] = $row2['monto_ayuda_social'];
                    $row['detalle_ayuda_social'] = $row2['detalle_ayuda_social'];
                }

                if( isset( $row['idinforme_ayuda_social'] ) ) {

                    $idinforme_ayuda_social = $row['idinforme_ayuda_social'];

                    // var_dump($idinforme_ayuda_social);

                    // Consultando tipos de ayuda del último informe:

                    // var_dump($tipoayuda_filtro_busqueda);

                    if( $tipoayuda_filtro_busqueda == '0' ) {

                        $resultado3 = $link->query("
                            SELECT * FROM tipo_ayuda_social
                            LEFT JOIN detalle_informe_ayuda_social ON detalle_informe_ayuda_social.idtipo_ayuda_social = tipo_ayuda_social.idtipo_ayuda_social
                            WHERE detalle_informe_ayuda_social.idinforme_ayuda_social = ".$idinforme_ayuda_social."
                        ");

                        $array_tipos_ayuda = [];
                        while( $row3 = mysqli_fetch_assoc( $resultado3 ) ) {
                            $array_tipos_ayuda[] = $row3['descripcion_tipo_ayuda_social'];
                        }     
                        $row['array_tipos_ayuda'] = $array_tipos_ayuda;


                        // Consultando cantidad de informes de ayuda social de cada jugador:
                        $resultado4 = $link->query("
                            SELECT COUNT(idinforme_ayuda_social) FROM informe_ayuda_social
                            WHERE informe_ayuda_social.idfichaJugador = ".$idfichaJugador."
                        ");

                        while( $row4 = mysqli_fetch_assoc( $resultado4 ) ) {
                            $row['cantidad_informes_ayuda_social'] = $row4['COUNT(idinforme_ayuda_social)'];
                        }                             
                        ...
                        while( $row3 = mysqli_fetch_assoc( $resultado3 ) ) {
                            ...
                            if( in_array($row3['idtipo_ayuda_social'], $tipoayuda_filtro_busqueda) ){
...
/* --------------------------------------------- Inicio de la función 'guardar_ayuda_social' --------------------------------------------- */
function guardar_ayuda_social( $datos ){
    include("conexion.php");
    $respuesta = "";
    $query = "";
    ...
    /*
    if( $datos['fecha_ayuda_social'] == '' ) {
        $datos['fecha_ayuda_social'] = date("Y-m-d");
    }
    */
    ...
    if( $datos['idinforme_ayuda_social'] == '' ) { //agregar nuevo informe
        
        $array_idfichaJugador = $datos['idfichaJugador'];
        $array_fecha_ayuda_social = $datos['fecha_ayuda_social'];
        $array_monto_ayuda_social = $datos['monto_ayuda_social'];
        $array_detalle_ayuda_social = $datos['detalle_ayuda_social'];
        // $array_input_ayuda_otro = $_POST['input_ayuda_otro'];


        for ($i=0; $i <count($array_idfichaJugador); $i++) { 
            
            $idfichaJugador = $array_idfichaJugador[$i];
            $fecha_ayuda_social = $array_fecha_ayuda_social[$i];
            $monto_ayuda_social = $array_monto_ayuda_social[$i];
            $detalle_ayuda_social = $array_detalle_ayuda_social[$i];

            if( $fecha_ayuda_social == '' ) {
                $fecha_ayuda_social = date("Y-m-d");
            }

            // $input_ayuda_otro = $array_input_ayuda_otro[$i];

            // ----------- Tabla 'informe_ayuda_social' ----------- //
            // INSERT:
            $query = $link->query("INSERT INTO informe_ayuda_social (
                idfichaJugador,
                fecha_ayuda_social,
                monto_ayuda_social,
                ...
                fecha_software
                  ...
                  '".utf8_decode($idfichaJugador)."',
                  '".utf8_decode($fecha_ayuda_social)."',
                  '".utf8_decode($monto_ayuda_social)."',
                  '".utf8_decode($detalle_ayuda_social)."',
                  '".utf8_decode($datos['nombre_usuario_software'])."',
                  '".getDateTime()."'
            )");

            // Si se insertan correctamente los datos en la tabla 'informe_ayuda_social', se procede a insertar datos en la tabla 'posicionCancha'
            if( $query )  { 

                // Valor del ID del informe de ayuda recién insertado:
                $idinforme_ayuda_social = $link->insert_id;

                // ------------------- EL USUARIO HA SELECCIONADO AL MENOS UN CHECKBOX ------------------- //
                if( isset( $datos['array_idtipo_ayuda'][$i] )  ) {

                    // Array que contiene los tipos de ayuda ingresados en el formulario:
                    $array_idtipo_ayuda = $datos['array_idtipo_ayuda'][$i];

                    // // Array que almacenará los datos que se registrarán en la BD:
                    $array_idtipo_ayuda_detalle = [];

                    for ( $j=0; $j < count($array_idtipo_ayuda); $j++ ) {
                        $idtipo_ayuda_social = $array_idtipo_ayuda[$j];

                        if( $idtipo_ayuda_social != '000' ) { // <---- Esto evita que agrege el valor del checkbox 'Otro'.
                            $array_idtipo_ayuda_detalle[] = $idtipo_ayuda_social;
                        }

                        // Si el usuario decidió agregar otro tipo de ayuda, entonces este se registrará en la tabla 'tipo_ayuda_social':
                        /*
                        if( $idtipo_ayuda_social == '000' ) {

                            $input_ayuda_otro = $datos['input_ayuda_otro'][$i];

                            if( $input_ayuda_otro != '' ) {

                                // ----------- Tabla 'tipo_ayuda_social' ----------- //
                                // INSERT:
                                $query = $link->query("INSERT INTO tipo_ayuda_social (
                                    descripcion_tipo_ayuda_social,
                                    nombre_usuario_software,
                                    fecha_software
                                    ) VALUES (
                                      '".utf8_decode($input_ayuda_otro)."',
                                      '".utf8_decode($datos['nombre_usuario_software'])."',
                                      '".getDateTime()."'
                                )");

                                if( $query ) {
                                    // Valor del ID del tipo de ayuda recién insertado:
                                    $idtipo_ayuda_social_otro = $link->insert_id;
                                    $array_idtipo_ayuda_detalle[] = $idtipo_ayuda_social_otro;
                                } else {
                    ...
                    // ---------------------------- Insertando en la tabla 'detalle_informe_ayuda_social' ---------------------------- //
                    for ( $k=0; $k< count($array_idtipo_ayuda_detalle); $k++ ) {

                        $idtipo_ayuda_social = $array_idtipo_ayuda_detalle[$k];

                        // ----------- Tabla 'detalle_informe_ayuda_social' ----------- //
                        // INSERT:
                        $query = $link->query("INSERT INTO detalle_informe_ayuda_social (
                            idinforme_ayuda_social,
                            idtipo_ayuda_social
                            ) VALUES (
                              '".utf8_decode($idinforme_ayuda_social)."',
                              '".utf8_decode($idtipo_ayuda_social)."'
                        )");
                    ...
                    $respuesta = 1; // INSERT ejecutado correctamente.
                // ------------------- EL USUARIO NO HA SELECCIONADO AL MENOS UN CHECKBOX ------------------- //
                }
        ...
        $idinforme_ayuda_social = $datos['idinforme_ayuda_social'];
        ...
        // ----------- Tabla 'informe_ayuda_social' ----------- //
        // UPDATE:
        $query = $link->query("UPDATE informe_ayuda_social SET
            fecha_ayuda_social = '".utf8_decode($datos['fecha_ayuda_social'])."',
            monto_ayuda_social = '".utf8_decode($datos['monto_ayuda_social'])."',
            detalle_ayuda_social = '".utf8_decode($datos['detalle_ayuda_social'])."',
            nombre_usuario_software = '".utf8_decode($datos['nombre_usuario_software'])."',
            fecha_software = '".getDateTime()."'
            WHERE idinforme_ayuda_social = ".$idinforme_ayuda_social."
        ");

        /*
        $debugging = "UPDATE informe_ayuda_social 
            fecha_ayuda_social = '".utf8_decode($datos['fecha_ayuda_social'])."',
            monto_ayuda_social = '".utf8_decode($datos['monto_ayuda_social'])."',
            detalle_ayuda_social = '".utf8_decode($datos['detalle_ayuda_social'])."',
            nombre_usuario_software = '".utf8_decode($datos['nombre_usuario_software'])."',
        ...
        */
        ...
        if( $query )  { 
            ...
            // ------------------- EL USUARIO HA SELECCIONADO AL MENOS UN CHECKBOX ------------------- //
            if( isset( $datos['array_idtipo_ayuda'] )  ) {

                // Array que contiene los tipos de ayuda ingresados en el formulario:
                $array_idtipo_ayuda = $datos['array_idtipo_ayuda'];

                // // Array que almacenará los datos que se registrarán en la BD:
                $array_idtipo_ayuda_detalle = [];

                for ( $i=0; $i < count($array_idtipo_ayuda); $i++ ) {
                    $idtipo_ayuda_social = $array_idtipo_ayuda[$i];
                    ...
                    if( $idtipo_ayuda_social == '000' ) {
                        ...
                        $input_ayuda_otro = $datos['input_ayuda_otro'];
                ...
                // Eliminando los detalles del informe:

                // ----------- DELETE a Tabla 'detalle_informe_ayuda_social' ----------- //
                $query = $link->query("DELETE FROM detalle_informe_ayuda_social WHERE idinforme_ayuda_social = ".$idinforme_ayuda_social."");

                if( $query ) {

                    // ---------------------------- Insertando en la tabla 'detalle_informe_ayuda_social' ---------------------------- //
                    for ( $i=0; $i < count($array_idtipo_ayuda_detalle); $i++ ) {

                        $idtipo_ayuda_social = $array_idtipo_ayuda_detalle[$i];
                        // ----------- Tabla 'detalle_informe_ayuda_social' ----------- //
                        // UPDATE:
                        $query = $link->query("INSERT INTO detalle_informe_ayuda_social (
                            idinforme_ayuda_social,
...
/* --------------------------------------------- Fin de la función 'guardar_ayuda_social' --------------------------------------------- */

/* --------------------------------------------- Fin de la función 'guardar_tipo_ayuda_otro' --------------------------------------------- */
function guardar_tipo_ayuda_otro( $datos ) {
    include("conexion.php");
    // ----------- Tabla 'tipo_ayuda_social' ----------- //
    // INSERT:
    $query = $link->query("INSERT INTO tipo_ayuda_social (
        ...
        fecha_software
          ...
          '".utf8_decode($datos['input_ayuda_otro'])."',
          '".utf8_decode($datos['nombre_usuario_software'])."',
          '".getDateTime()."'
    )");
